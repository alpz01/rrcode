...
function vidStream() {
    ...
    for (let i = 1; i <= count; i++) {
        const link = srcTypeHandlers[srcType](i);
...
document.getElementById("nextEp").addEventListener("click", () => {
    ...
    if (btnNextSpamCount === 0) {
        if ((epSet[0] === "cf" && btnNextEpCount < number) || (epSet[0] !== "cf" && btnNextEpCount < epSet[1])) {
...
function getNextEp() {
    ...
    if (player === "cf" && btnNextEpCount < number) {
        btnNextEpCount = start + 1;
        return Math.abs((btnNextEpCount - number) + 1);
    } else if (player !== "cf" && btnNextEpCount < start) {

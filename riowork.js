...
function forJwPlayer(input) {
    ...
    jwplayer.key = "ITWMv7t88JGzI0xPwW8I0+LveiXX9SWbfdmt0ArUSyc=", jwplayer("ahesuifu").setup({
        image: jwImage,
        aspectratio: "16:9",
        width: '100%',
...
let count;
try {
    count = parseInt(epSet[1])
}
catch (e) {
    (e instanceof ReferenceError) ? count = 0 : count = parseInt(epSet[1])
}
...
function checkFor(trgt) {
    ...
    function fpoptlck() {
        let z = document.getElementById("fpopt")
...
function downloadEp() {
    ...
    if (trgt == "g") {
        location.href = `https://drive.google.com/u/0/uc?id=${episodes[btnClick-1]}&export=download`; 
    } else {
